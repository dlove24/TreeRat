...
domainname guess_zone_name (const char* file) {
  ...
  if (tolower (ptr[0]) == 'd' &&
    ...
    if (strlen (ptr) - 4 >= 256) {
      throw PException ("File name too long!");
      }
...
void read_master_file (const char* file, domainname& znroot, void* userdat,
  ...
  if (!f) {
    throw PException (true, "Could not open %s", file);
    }
    ...
    while (!feof (f)) {
        ...
        if (type->type != DNS_TYPE_SOA) {
          if (! (flags & POSLIB_MF_NOSOA)) {
            ...
            if (rr_cb) {
              ...
              rr.RDATA = (char*) memdup (tmp.c_str(), tmp.size());
              rr_cb (userdat, &rr);
              }
      ...
      if (rrl_cb) {
        try {
          ...
          if (origin != znroot) {
            t2 = rrdata_convertdoms (type, znroot, origin, ptr);
            }
          }
