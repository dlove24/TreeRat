...
int dom_comprlen (message_buff& buff, int ix) {
  int len = 0;
  char x;
  char* ptr = buff.msg + ix, *end = buff.msg + buff.len;

  while (true) {
    if (ptr >= end) {
      throw PException ("Domain name exceeds message borders");
      }
    ...
    if ( (*ptr & 192) == 192) {
      if (ptr + 1 >= end) {
        throw PException ("Compression offset exceeds message borders");
        }
    ...
    if (x != 0) {
      throw PException ("Unknown domain label type");
      }
    ...
    if (len >= 255) {
      throw PException ("Domain name too long");
      }
    }
  }
  ...
  while (true) {
    ...
    if ( (*ptr & 192) == 192) {
      if (++reclevel >= DOM_RECLEVEL) {
        throw PException ("Max dom recursion level reached");
        }
      ...
      if (val >= (ptr - buff.msg)) {
        throw PException ("Bad compression offset");
        }
...
dom_compr_info::dom_compr_info (_domain _dom, int _ix, int _nl, int _nul) {
  dom = _dom;
  ix = _ix;
  nl = _nl;
  nul = _nul;
...
int dom_partiallength (_domain dom, int n) {
  int len = 0;
...
void dom_write (stl_string& ret, _domain dom, stl_slist (dom_compr_info) &comprinfo) {
  //  ret.append(dom, domlen(dom));
  ...
  if (best == comprinfo.end()) {
    ...
    char val;
    ret.append (dom, dom_partiallength (dom, nlabels - best->nl));
    val = (best->ix / 256) | 192;
    ret.append (&val, 1);
    val = best->ix;
    ret.append (&val, 1);
...
int domlen (const _domain _dom) {
  char* dom = _dom;
  int len = 1;

  while (*dom) {
    if (*dom > 63) {
      throw PException (true, "Unknown domain nibble %d", *dom);
      }
    ...
    if (len > 255) {
      throw PException ("Length too long");
      }
    }
...
bool domcmp (const _domain _dom1, const _domain _dom2) {
...
domainname::domainname (const char* string, domainname origin) {
  char tmp[DOM_LEN];
...
domainname::domainname (const char* string, const _domain origin) {
  char tmp[DOM_LEN];
...
domainname::domainname (bool val, const _domain dom) {
  domain = domdup (dom);
  }
...
domainname& domainname::operator+= (const domainname& nam) {
  ...
  if (lenres + lensrc - 1 > DOM_LEN) {
    throw PException ("Domain name too long");
    }

  domain = (char*) realloc (domain, lenres + lensrc - 1);
...
void domcat (_domain res, const _domain src) {
...
void domfromlabel (_domain dom, const char* label, int len) {
  ...
  if (len > DOMLABEL_LEN) {
    throw PException (true, "Domain name label %s too long", label);
    }
...
int dom_nlabels (const _domain _dom) {
  char* dom = _dom;
  int n_labels = 1;
...
stl_string dom_label (const _domain _dom, int label) {
  stl_string ret;
  char* dom = _dom;
...
const _domain dom_plabel (const _domain dom, int label) {
  ...
  if (label < 0) {
    throw PException ("Negative label accessed");
    }

  while (label--) {
    if (*ret == 0) {
      throw PException ("Label not in domain name");
      }
...
stl_string dom_tostring (const _domain _dom) {
  char* dom = _dom;
...
int domncommon (const _domain _dom1, const _domain _dom2) {
  char* dom1 = (char*) _dom1,
        *dom2 = (char*) _dom2;
