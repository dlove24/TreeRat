...
bool txt_to_bool (const char* buff) {
  if (strcmpi (buff, "yes") == 0) {
    return true;
    }

  if (strcmpi (buff, "true") == 0) {
    return true;
    }
  ...
  if (strcmpi (buff, "on") == 0) {
    return true;
    }

  if (strcmpi (buff, "yo") == 0) {
    return true;
    }
  ...
  if (strcmpi (buff, "no") == 0) {
    return false;
    }

  if (strcmpi (buff, "false") == 0) {
    return false;
    }
  ...
  if (strcmpi (buff, "off") == 0) {
    return false;
    }

  if (strcmpi (buff, "nope") == 0) {
    return false;
    }

  if (strcmpi (buff, "never") == 0) {
    return false;
    }
...
int txt_to_int_internal (const char* _buff, bool support_negative) {
  ...
  if (*buff == '-') {
    if (!support_negative) {
      throw PException (true, "Negative number not supported: %s", _buff);
      }
...
int txt_to_ip (char ip[4], const char* _buff, bool do_portion) {
  char* buff = (char*) _buff;
  ...
  while (buff[p] != '\0') {
    if (isdigit (buff[p])) {
      ...
      if (node > 255) {
        throw PException ("IP node value exceeds 255");
        }
      }
      ...
      if (buff[p] == '.') {
        ...
        if (tmp >= 3) {
          throw PException ("More than three dots in IP number");
          }
  ...
  if (tmp != 4 && !do_portion) {
    throw PException ("Not enough nodes in IP number");
    }
...
void txt_to_iprange (char* iprange, const char* val) {
  ...
  if ( (ptr = strchr ( (char*) val, '/')) != NULL) {
    if (strchr (ptr, '.')) {
      ...
      if (x > 128) {
        throw PException ("IPv6 mask value too long");
        }
    ...
    if ( (ptr - val) >= (signed) sizeof (buff)) {
      throw PException ("Ip number too long");
      }
    ...
    for (x = txt_to_ip (iprange + 4, val, true) - 1; x >= 0; x--) {
      iprange[x] = 255;
      }
    }
  }
...
bool iprange_matches (char* iprange, char* ip) {
  for (int x = 0; x < 4; x++) if ( (ip[x] ^ iprange[x + 4]) & iprange[x]) {
      return false;
      }
...
int txt_to_ipv6 (char ipv6[16], const char* buff, bool do_portion) {
  ...
  while (buff[++pos] != '\0') {
    if (buff[pos] == '.') {
      /* an imbedded ipv4 */
      if (node + 2 > 8) {
        throw PException ("No room for embedded IPv4 in IPv6 address");
        }
    ...
    if (buff[pos] == ':') {
      ...
      if (pos) {
        if (nodeval == -1) {
          throw PException ("IPv6 address has empty node value");
          }
        ...
        if (buff[pos + 1] != ':') {
          throw PException ("IPv6 address should have ::");
          }
      ...
      if (node > 7) {
        throw PException ("IPv6 address has too much nodes");
        }
      ...
      if (chval < 0) {
        throw PException ("Incorrect hex in IPv6 address");
        }
      ...
      if (nodeval > 65535) {
        throw PException ("IPv6 node val too large");
        }
      }
    }
  ...
  if (multigroup_pos != -1) {
    ...
    if (node < 7) {
      throw PException ("Too less nodes in IPv6 address");
      }
    }
...
bool ip6range_matches (char* iprange, char* ip) {
  for (int x = 0; x < 16; x++) if ( (ip[x] ^ iprange[x + 16]) & iprange[x]) {
      return false;
      }
...
bool in_addrrange_list (stl_list (addrrange) &lst, _addr* a) {
  ...
  while (it != lst.end()) {
    if (addrrange_matches (it->range, a)) {
      return true;
      }
...
bool in_addr_list (stl_slist (_addr) &lst, _addr* a, bool match_port) {
  ...
  while (it != lst.end()) {
    if (match_port) {
      if (addrport_matches (&*it, a)) {
        return true;
        }
      }

    else {
      if (address_matches (&*it, a)) {
        return true;
        }
      }
...
void txt_to_email (char* target, const char* src, const _domain origin) {
  char dom[DOM_LEN];
  char* cptr;
  ...
  if ( (cptr = (char*) strchr (src, '@')) != NULL && ! (cptr[0] == '@' && cptr[1] == 0)) {
    ...
    if (src[0] == '@') {
      throw PException ("Incorrect email adress/domain name: begins with @");
      }
...
void txt_to_dname (char* target, const char* src, const _domain origin) {
  char* ptr;
  char label[DOM_LEN];
  char tmp[16];
  char hex;
  ...
  while (*src) {
    if (src[0] == '.' && src[1] != '\0') {
      ...
      if (strchr (src + 1, ':')) {
        if (domlen (target) + 42 >= DOM_LEN) {
          throw PException ("IPv6 domainname doesn't fit");
          }
        ...
        domcat (target, "\3ip6\3int");
        return;
        }

      else {
        /* ipv4 */
        if (domlen (target) + 14 >= DOM_LEN) {
          throw PException ("IPv6 domainname doesn't fit");
          }
        ...
        for (ttmp = ret - 1; ttmp >= 0; ttmp--) {
          sprintf (tmp + 4, "%d", tmp[ttmp]);
          domfromlabel (target + domlen (target) - 1, tmp + 4);
          }

        domcat (target, "\7in-addr\4arpa");
        return;
        }
      }
    ...
    if (ptr) {
      if (ptr == src) {
        throw PException ("Zero length label");
        }
...
void txt_to_addr (_addr* ret, const char* addr, int default_port, bool client) {
  ...
  if (ptr) {
    if ( (unsigned) (ptr - addr) > (unsigned) sizeof (taddr)) {
      throw PException ("Address too long");
      }
...
u_int32 poslib_degstr (char*& src, char pre, char post) {
  ...
  if (toupper (tmp[0]) == post) {
    ...
    throw PException (true, "Malformed LOC RR: expected '%c' or '%c', got %s", pre, post, tmp.c_str());
    }

  return ret;
  }

char poslib_loc_precision (const char* str) {
  int x, y = 0;
  int n = 0;

  if (sscanf (str, "%d.%dm", &x, &y) < 1) {
    throw PException (true, "Invalid precision: %s", str);
    }
...
void txt_to_loc (char* res, char*& src) {
  ...
  if (sscanf (tmp.c_str(), "%d.%dm", &x, &y) <= 0) {
    throw PException ("Invalid altitude");
    }
...
stl_string str_qtype (u_int16 qtype) {
  ...
  if (qtype == QTYPE_MAILB) {
    return "MAILB";
    }

  if (qtype == QTYPE_MAILA) {
    return "MAILA";
    }
...
stl_string str_rcode (int rcode) {
  ...
  if (rcode == RCODE_QUERYERR) {
    return "QUERYERR";
    }

  if (rcode == RCODE_SRVFAIL) {
    return "SRVFAIL";
    }
