...
int udpread (int sockid, char* buff, int len, _addr* addr) {
  ...
  int ret = recvfrom (sockid, buff, len, 0, (sockaddr*) addr, &addr_size);
...
void udpsend (int sockid, char* buff, int len, _addr* addr) {
  if (sendto (sockid, buff, len, 0, (sockaddr*) addr, struct_len (addr)) < 0) {
...
int tcpaccept (int socket, _addr* askaddr) {
  ...
  if (ret < 0) {
    throw PException ("Could not accept TCP connection");
    }

  if (askaddr) {
    memcpy (askaddr, &querier, len);
    }

  return ret;
  }

int tcpsend (int socket, char* buff, int bufflen) {
  int ret = send (socket, buff, bufflen, 0);

  if (ret < 0 && errno != EAGAIN) {
    throw PException ("Could not send TCP message");
    }

  return ret;
  }

int tcpread (int socket, char* buff, int bufflen) {
  int ret = recv (socket, buff, bufflen, 0);
...
void tcpsendall (int sockid, char* buff, int len, int maxtime) {
  ...
  while (len > 0) {
    while ( (cur = getcurtime()) <= end) {
      ...
      if (set.canwrite (0) || posclient_quitflag) {
        break;
        }
      }

    if (!set.canwrite (0)) {
      throw PException ("Could not send buffer");
      }
...
void tcpreadall (int sockid, char* buff, int len, int maxtime) {
  ...
  while (len > 0) {
    while ( (cur = getcurtime()) <= end) {
      ...
      if (set.isdata (0) || posclient_quitflag) {
        break;
        }
      }
    ...
    if (ret == 0) {
      throw PException ("TCP client hung up!");
      }
...
void getaddress_ip4 (_addr* res, const char* ipv4_data, int port) {
...
#ifdef HAVE_IPV6
void getaddress_ip6 (_addr* res, const char* ipv6_data, int port) {
  memset (res, 0, sizeof (_addr));
#ifdef HAVE_SIN6_LEN
...
void getaddress (_addr* res, const char* ip, int port) {
  ...
  if (ptr) {
    ...
    ( (sockaddr_in6*) res)->sin6_family = AF_INET6;
    ( (sockaddr_in6*) res)->sin6_port = htons (port);
    txt_to_ipv6 ( (char*) & ( (sockaddr_in6*) res)->sin6_addr, ip);
    return;
    }
  ...
  ( (sockaddr_in*) res)->sin_family = AF_INET;
  ( (sockaddr_in*) res)->sin_port = htons (port);
  txt_to_ip ( (char*) & ( (sockaddr_in*) res)->sin_addr, ip);
  }
...
bool address_matches (_addr* addr1, _addr* addr2) {
  if (addr1->s_family != addr2->s_family) {
    return false;
    }
