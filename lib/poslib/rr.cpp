...
rr_type* rrtype_getinfo (u_int16 type) {
  ...
  for (t = 0; t < n_rr_types; t++)
    if (rr_types[t].type == type) {
      return (rr_type*) &rr_types[t];
      }
...
rr_type* rrtype_getinfo (const char* name) {
  ...
  for (t = 0; t < n_rr_types; t++)
    if (strcmpi (rr_types[t].name, name) == 0) {
      return (rr_type*) &rr_types[t];
      }
...
uint16_t qtype_getcode (const char* name) {
  try {
    return txt_to_int (name);
    }

  catch (PException p) {
    if (strcmpi (name, "maila") == 0) {
      return QTYPE_MAILA;
      }

    if (strcmpi (name, "mailb") == 0) {
      return QTYPE_MAILB;
      }

    if (strcmpi (name, "ixfr") == 0) {
      return QTYPE_IXFR;
      }

    if (strcmpi (name, "axfr") == 0) {
      return QTYPE_AXFR;
      }

    if (strcmpi (name, "any") == 0) {
      return QTYPE_ANY;
      }
    ...
    if (!t) {
      throw PException (true, "Qtype %s not supported", name);
      }
...
int rr_len (char prop, message_buff& buff, int ix, int len) {
  ...
  switch (prop) {
    ...
    case 'h': /* character strings */
      ...
      while (ptr - buff.msg  - ix < len) {
        ptr += *ptr + 1;
        }

      if (ptr != buff.msg + ix + len) {
        throw PException ("Character strings too long for RR");
        }
    ...
    case 'w': /* well-known services */
      if (len < 5) {
        throw PException ("WKS RR too long for RR");
        }
    ...
    case '7': /* ipv6 address + prefix */
      ...
      if (ix + x + 1 >= len) {
        throw PException ("A6 too long for RR");
        }
    ...
    case 'o': /* DNS LOC */
      if (buff.msg[ix] != 0) {
        throw PException ("Unsupported LOC version");
        }
...
void rr_read (u_int16 RRTYPE, char*& RDATA, uint16_t& RDLEN, message_buff& buff, int ix, int len) {
  ...
  if (ix + len > buff.len) {
    throw PException ("RR doesn't fit in DNS message");
    }
  ...
  if (info) {
    ...
    try {
      ...
      while (*ptr) {
        ...
        if (x > len) {
          throw PException ("RR item too long!");
          }

        if (*ptr == 'd' || *ptr == 'm') {
          /* domain name: needs to be decompressed */
          dom = dom_uncompress (buff, ix);
          res.append (dom, domlen (dom));
          free (dom);
          }

        else {
          res.append (buff.msg + ix, x);
          }
      ...
      if (len != 0) {
        throw PException ("extra data in RR");
        }
      }
    ...
    if (len != 0) {
      throw PException ("RR length too long");
      }
    }

  else {
    /* we do not support the RR type: just copy it altogether */
    res.append (buff.msg + ix, len);
    }

  RDLEN = res.length();
  RDATA = (char*) memdup ( (void*) res.c_str(), res.length());
  }
...
void rr_write (u_int16 RRTYPE, char* RDATA, uint16_t RDLEN, stl_string& dnsmessage, stl_slist (dom_compr_info) &comprinfo) {
  ...
  if (!info || ! (info->flags & R_COMPRESS)) {
    dnsmessage.append (RDATA, RDLEN);
    return;
    }
  ...
  while (*ptr) {
    ...
    if (*ptr == 'd' || *ptr == 'm') {
      ...
      dnsmessage.append (RDATA + ix, len);
      }
...
stl_string rr_tostring (u_int16 RRTYPE, const char* _RDATA, int RDLENGTH) {
  rr_type* info = rrtype_getinfo (RRTYPE);

  char buff[128];
  char* ptr, *RDATA = (char*) _RDATA;
  stl_string ret;
  int x, y;

  if (!info) {
    return "<unknown rr type>";
    }
  ...
  while (*ptr) {
    switch (*ptr) {
      ...
      case 'c': // character string
        ret.append ("\"");
        ret.append (RDATA + 1, (int) *RDATA);
        ret.append ("\"");
        RDATA += *RDATA + 1;
        break;

      case 'h': // character strings
        while (RDATA < (_RDATA + RDLENGTH)) {
          ret.append ("\"");
          ret.append (RDATA + 1, (int) *RDATA);
          ret.append ("\" ");
          RDATA += *RDATA + 1;
...
void read_line (char* buff, FILE* f, int* linenum, int* linenum_old, int buffsz) {
  ...
  while (!feof (f)) {
    ...
    switch (c) {
      ...
      case '"':
        if (pos > buffsz - 2) {
          throw PException ("Line too long");
          }
        ...
        do {
          ...
          if (feof (f)) {
            throw PException ("EOF in quotes");
            }
...
stl_string read_entry (char*& data) {
  ...
  if (*tmp == 0) {
    throw PException ("Unexpected end-of-line");
    }
  ...
  while (*tmp != '\0' && (in_quote || (*tmp != ' ' && *tmp != '\t'))) {
    ...
    if (len >= sizeof (buff) - 1) {
      throw PException ("Data too long!");
      }
...
stl_string rr_fromstring (u_int16 RRTYPE, const char* _data, _domain origin) {
  ...
  if (!info) {
    throw PException ("Unknown RR type");
    }
  ...
  while (*ptr) {
    switch (*ptr) {
      ...
      case 'm':
        ...
        ret.append (tdom.c_str(), tdom.len());
        break;

      case 'i':
        tmp = read_entry (data);
        txt_to_ip (buff, (char*) tmp.c_str());
        ret.append (buff, 4);
        break;
      ...
      case 's':
        ...
        ret.append (uint16_buff (val), 2);
        break;

      case 'l':
        tmp = read_entry (data);
        val = txt_to_int ( (char*) tmp.c_str());
        ret.append (uint32_buff (val), 4);
        break;

      case '6':
        tmp = read_entry (data);
        txt_to_ipv6 (buff, (char*) tmp.c_str());
        ret.append (buff, 16);
        break;
      ...
      case 'c':
        ...
        if (tmp.size() > 63) {
          throw PException ("Character string too long");
          }
      ...
      case 'w':
        ...
        while (*data) {
          ...
          if (val >= (signed) sizeof (buff) * 8) {
            throw PException (true, "Port number %d too large", val);
            }
  ...
  if (*data) {
    throw PException ("Extra data on RR line");
    }
...
void rr_goto (char*& RDATA, u_int16 RRTYPE, int ix) {
  rr_type* info = rrtype_getinfo (RRTYPE);
  ...
  for (int x = 0; x < ix; x++) {
    if (ptr[x] == '\0') {
      throw PException ("RR does not contain that property");
      }
...
_domain rr_getbindomain (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  rr_goto (RDATA, RRTYPE, ix);
  return domdup (RDATA);
  }

domainname rr_getdomain (const char* RDATA, u_int16 RRTYPE, int ix) {
  domainname dom;
  char* ptr = rr_getbindomain (RDATA, RRTYPE, ix);
  dom = domainname (true, ptr);
  free (ptr);
  return dom;
  }

char* rr_getbinmail (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  return rr_getbindomain (RDATA, RRTYPE, ix);
  }

domainname rr_getmail (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  return rr_getdomain (RDATA, RRTYPE, ix);
  }

u_int16 rr_getshort (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  rr_goto (RDATA, RRTYPE, ix);
  return RDATA[0] * 256 + RDATA[1];
  }

u_int32 rr_getlong (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  rr_goto (RDATA, RRTYPE, ix);
  return RDATA[0] * 16777216 + RDATA[1] * 65536 + RDATA[2] * 256 + RDATA[3];
  }

char* rr_getip4 (const char* _RDATA, u_int16 RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  rr_goto (RDATA, RRTYPE, ix);
...
char* rr_getip6 (const char* _RDATA, int RRTYPE, int ix) {
  char* RDATA = (char*) _RDATA;
  rr_goto (RDATA, RRTYPE, ix);
